...
const TimezoneHeader = ({ userCountry, t }) => {
  ...
  const daylightTimeNotice = useMemo(() => {
    ...
    const fallback = dstPhrasesByLanguage[currentLanguage] || dstPhrasesByLanguage['en'];
    const translatedPhrases = {
      starts: t('dstDayStartsLabel') || fallback.starts,
      ends: t('dstDayEndsLabel') || fallback.ends,
      reverts: t('dstRevertsLabel') || fallback.reverts,
      europe: t('dstEuropeLabel') || fallback.europe,
      newZealand: t('dstNewZealandLabel') || fallback.newZealand,
    };
  ...
  const getUserLocalInfo = () => {
    ...
    if (Array.isArray(timezoneData)) {
      return timezoneData.map(({ name, timezone }) => {
        ...
        const timezoneCode = getTimezoneCode(name, timezone);
  ...
  const desktopTimeFontSize = '0.85rem';
  const desktopCompactTimeFontSize = '0.8rem';
  const desktopLabelFontSize = '0.7rem';
  const desktopCompactLabelFontSize = '0.65rem';
  const desktopUltraCompactLabelFontSize = '0.62rem';
    ...
    return (
      <div style={{
        ...
        <div style={{
          ...
          <span style={{ fontSize: isMobile ? '0.75rem' : desktopLabelFontSize, color: '#5a6c7d', fontWeight: '600' }}>{`${singaporeTimezoneCode} (${singaporeUTCLabel})`}</span>
  ...
  if (shouldUseCompactDesktopLayout && Array.isArray(displayTimezones)) {
    const timezoneCards = [
      ...displayTimezones.map((info) => ({
        ...
        label: getTimezoneDisplayWithUTC(info),
        time: info.localTime,
        date: info.localDate,
        diff: info.timeDiffText.replace('hours', 'h').replace('hour', 'h'),
        isSameTime: info.isSameTime,
        isSingapore: false,
      })),
      {
        key: 'sgt-reference',
        flag: '🇸🇬',
        label: `${singaporeTimezoneCode} (${singaporeUTCLabel})`,
        time: singaporeTime,
        date: singaporeDate,
        diff: '',
        isSameTime: true,
      ...
      <div style={{
        ...
        {rows.map((row, rowIndex) => (
          <div
            ...
            style={{
              display: 'flex',
              gap: shouldUseDenseCompactCards ? '3px' : '5px',
              justifyContent: 'center',
              width: '100%'
            }}
          >
            {row.map((card) => (
              <div key={card.key} style={{
                ...
                alignItems: 'center',
                gap: '2px',
                padding: shouldUseDenseCompactCards ? '3px 4px' : '4px 6px',
                backgroundColor: shouldUseDenseCompactCards
                  ? (card.isSingapore ? '#fff8ee' : '#eef7fa')
                  : (card.isSingapore ? '#fff3e0' : '#e8f4f8'),
                borderRadius: shouldUseDenseCompactCards ? '4px' : '4px',
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '3px', minWidth: 0, maxWidth: '100%' }}>
                  ...
                  <span style={{
                    fontSize: desktopUltraCompactLabelFontSize,
                    color: '#5a6c7d',
                    fontWeight: '600',
                    textAlign: 'center',
                    overflowWrap: 'anywhere',
                ...
                <span style={{ fontSize: desktopCompactTimeFontSize, fontWeight: '700' }}>{card.time}</span>
                <span style={{ fontSize: '0.58rem', color: '#5a6c7d' }}>{card.date}</span>
                {!card.isSingapore ? (
                  <span style={getBoundedDiffTextStyle({ fontSize: '0.47rem', color: card.isSameTime ? '#27ae60' : '#7f8c8d', fontStyle: 'italic' })}>
                    {card.diff}
                  </span>
                ) : null}
              </div>
        ...
        Array.isArray(displayTimezones) ? (
          ...
          shouldUseGridLayout ? (
            isAustralia ? (
                ...
                <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: isMobile ? '6px' : '6px', justifyContent: 'center', width: '100%', flexWrap: 'wrap' }}>
                  {displayTimezones.map((info, index) => (
                    <div key={index} style={{
                      ...
                      <span style={{ fontSize: isMobile ? '0.85rem' : desktopTimeFontSize, fontWeight: '700' }}>{info.localTime}</span>
                      <span style={{ fontSize: isMobile ? '0.7rem' : '0.65rem', color: '#5a6c7d' }}>{info.localDate}</span>
                      <span style={getBoundedDiffTextStyle({ fontSize: isMobile ? '0.58rem' : '0.55rem', color: info.isSameTime ? '#27ae60' : '#7f8c8d', fontStyle: 'italic' })}>
              ...
              <>
                <div style={{ display: 'flex', gap: isMobile ? '3px' : '6px', justifyContent: 'center', width: '100%', flexWrap: 'wrap' }}>
                  {displayTimezones.slice(0, 5).map((info, index) => (
                    <div key={index} style={{
                      ...
                      padding: isMobile ? '4px 4px' : '5px 6px',
                      backgroundColor: '#e8f4f8',
                      borderRadius: isMobile ? '4px' : '4px',
                      border: '1px solid #b3d9e6',
                      whiteSpace: 'normal',
                      flex: isMobile ? `0 1 calc(${100/5}% - 3px)` : '1',
                      minWidth: isMobile ? '65px' : '85px',
                      maxWidth: isMobile ? '90px' : '110px',
                      overflow: 'hidden',
                    }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '1px', justifyContent: 'center', width: '100%' }}>
                        <span style={{ fontSize: isMobile ? '0.75rem' : '0.8rem', flexShrink: 0 }}>{countryFlags[userCountry] || '🌍'}</span>
                        <span style={{ 
                          ...
                          {getMobileOptimizedTimezoneDisplay(info)}
                        </span>
                      </div>
                      <span style={{ fontSize: isMobile ? '0.68rem' : '0.75rem', fontWeight: '700', lineHeight: '1' }}>{info.localTime}</span>
                      <span style={{ fontSize: isMobile ? '0.5rem' : '0.55rem', color: '#5a6c7d', lineHeight: '1' }}>{info.localDate}</span>
                      <span style={getBoundedDiffTextStyle({ fontSize: isMobile ? '0.42rem' : '0.47rem', color: info.isSameTime ? '#27ae60' : '#7f8c8d', fontStyle: 'italic', lineHeight: '1' })}>
                        {info.timeDiffText.replace('hours', 'h').replace('hour', 'h')}
                      </span>
                    </div>
                  ))}
                ...
                <div style={{ display: 'flex', gap: isMobile ? '3px' : '6px', justifyContent: 'center', alignItems: 'stretch', width: '100%', flexWrap: 'wrap' }}>
                  <div style={{
                    display: 'flex',
                    flexDirection: 'column',
                    alignItems: 'center',
                    ...
                    border: '1px solid #b3d9e6',
                    whiteSpace: 'normal',
                    flex: isMobile ? `0 1 calc(${100/5}% - 3px)` : 'none',
                    minWidth: isMobile ? '65px' : '85px',
                    maxWidth: isMobile ? '90px' : '110px',
                    overflow: 'hidden',
                  }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '1px', justifyContent: 'center', width: '100%', minWidth: 0 }}>
                      <span style={{ fontSize: isMobile ? '0.7rem' : '0.8rem', flexShrink: 0 }}>{countryFlags[userCountry] || '🌍'}</span>
                      <span style={{ 
                        ...
                        {getMobileOptimizedTimezoneDisplay(displayTimezones[10])}
                      </span>
                    </div>
                    <span style={{ fontSize: isMobile ? '0.65rem' : '0.75rem', fontWeight: '700', lineHeight: '1.05' }}>{displayTimezones[10].localTime}</span>
                    <span style={{ fontSize: isMobile ? '0.45rem' : '0.55rem', color: '#5a6c7d', lineHeight: '1' }}>{displayTimezones[10].localDate}</span>
                    <span style={getBoundedDiffTextStyle({ fontSize: isMobile ? '0.42rem' : '0.47rem', color: displayTimezones[10].isSameTime ? '#27ae60' : '#7f8c8d', fontStyle: 'italic', lineHeight: '1' })}>
                  ...
                  <div style={{
                    ...
                    <span style={{ fontSize: isMobile ? '0.65rem' : '0.75rem', fontWeight: '700', lineHeight: '1.05' }}>{singaporeTime}</span>
                    <span style={{ fontSize: isMobile ? '0.45rem' : '0.55rem', color: '#5a6c7d', lineHeight: '1' }}>{singaporeDate}</span>
                  </div>
                </div>
              </>
            ) : (
          ...
          <>
            <div style={{
              ...
              <span style={{ fontSize: isMobile ? '0.85rem' : desktopTimeFontSize, fontWeight: '700' }}>{displayTimezones.localTime}</span>
              <span style={{ fontSize: isMobile ? '0.7rem' : '0.65rem', color: '#5a6c7d' }}>{displayTimezones.localDate}</span>
              <span style={getBoundedDiffTextStyle({ fontSize: isMobile ? '0.58rem' : '0.55rem', color: displayTimezones.isSameTime ? '#27ae60' : '#7f8c8d', fontStyle: 'italic', marginLeft: isMobile ? '0' : '4px' })}>
