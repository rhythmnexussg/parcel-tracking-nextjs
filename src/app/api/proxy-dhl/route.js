...
export async function GET(request) {
  const limited = rateLimit(request, { keyPrefix: 'proxy-dhl', maxRequests: 25, windowMs: 60 * 1000 });
  ...
  try {
    ...
    if (!/^[A-Za-z0-9-]{6,50}$/.test(trackingNumber)) {
      return secureApiResponse(NextResponse.json({ error: 'Invalid trackingNumber format' }, { status: 400 }));
    }
